...
if os.name == 'nt':
	...
	metar_path = 'D:\AS\current\metar.txt'
...
def get_metar_press():
	...
	global pressure
	global metar_path 
	my_metar = os.path.isfile(metar_path)
	if my_metar:
...
def tabela():
	...
	if (diff_t > 1):
		...
		"""
		print "\033c"+" Flight info -----------|-------|Pred. closest   |- Current Az/Alt ----|--- Transits:", vm.name, moon_az, moon_alt,'  &  ', vs.name, sun_az, sun_alt 
		...
		for pentry in plane_dict:
				if (plane_dict[pentry][5] <= 250):
						...
						if (-transit_separation_GREENALERT_FG < separation_deg < transit_separation_GREENALERT_FG):
							wiersz += '{} {:>6} {}'.format(GREENALERT, str(plane_dict[pentry][24]-plane_dict[pentry][23]),RESET) ## SEPARACJA
							...
							wiersz += '{} {:>6} {}'.format(REDALERT, str(plane_dict[pentry][24]-plane_dict[pentry][23]),RESET) ## SEPARACJA
							wiersz += '{:>8}'.format(str(plane_dict[pentry][27]))  ## DISTANCE: AIRPLANE POS TO AIRPLANE PATH CROSS  
							wiersz += '{:>7}'.format(str(round(plane_dict[pentry][25],1))) ## DISTANCE MY_POS TO CROSS POINT
						...
						if (-transit_separation_GREENALERT_FG < separation_deg2 < transit_separation_GREENALERT_FG):
							wiersz += '{} {:>6} {}'.format(GREENALERT, str(plane_dict[pentry][19]-plane_dict[pentry][18]),RESET) ## SEPARACJA
							...
							wiersz += '{} {:>6} {}'.format(REDALERT, str(plane_dict[pentry][19]-plane_dict[pentry][18]),RESET) ## SEPARACJA
							wiersz += '{:>8}'.format(str(plane_dict[pentry][21]))  ## DISTANCE: AIRPLANE POS TO AIRPLANE PATH CROSS  
							wiersz += '{:>7}'.format(str(round(plane_dict[pentry][20],1))) ## DISTANCE MY_POS TO CROSS POINT
							...
							wiersz += '{} {:>6} {}'.format(RED, str(plane_dict[pentry][19]-plane_dict[pentry][18]),RESET) ## SEPARACJA
							wiersz += '{:>8}'.format(str(plane_dict[pentry][21]))  ## DISTANCE: AIRPLANE POS TO AIRPLANE PATH CROSS  
							wiersz += '{:>7}'.format(str(round(plane_dict[pentry][20],1))) ## DISTANCE MY_POS TO CROSS POINT
							wiersz += '{:>10}'.format(str(plane_dict[pentry][22]))			## delta_tim	## TIME UNTIL PLANE ARRIVE AT CROSS POINT	
